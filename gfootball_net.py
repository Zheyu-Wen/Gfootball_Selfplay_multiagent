...
from gfootball_net_architecture import gfootball_net as gfnet
...
class NNetWrapper(NeuralNet):
    ...
    def train(self, examples):
        ...
        for epoch in range(args.epochs):
            ...
            while batch_idx < int(len(examples)/args.batch_size):
                ...
                agent_num, obs, pis, vs, next_obs = list(zip(*[examples[i] for i in sample_ids]))
                ...
                # predict
                if args.cuda:
                    obs, target_pis, target_vs = obs.contiguous().cuda(), target_pis.contiguous().cuda(), target_vs.contiguous().cuda()
    ...
    def predict(self, obs):
        ...
        # preparing input
        board = torch.FloatTensor(obs.astype(np.float64))
        if args.cuda: obs = obs.contiguous().cuda()
        obs = obs.reshape(-1, 4, self.obs_width, self.obs_height)
